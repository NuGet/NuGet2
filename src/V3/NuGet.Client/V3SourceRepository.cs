...
namespace NuGet.Client
{
    public class V3SourceRepository : SourceRepository
    {
        ...
        {
            List<string> frameworkShortNames = new List<string>();
            foreach(FrameworkName fx in filters.SupportedFrameworks)
            {
                frameworkShortNames.Add(VersionUtility.GetShortFrameworkName(fx));
            }
            return await _client.Search(searchTerm, frameworkShortNames, filters.IncludePrerelease, skip, take, cancellationToken);
