...
namespace NuGet.Resolution
{
    ...
    internal class CombinationSolver<T>
    {
        private T[] solution;
                                                       ...
                                                       Func<T, T, bool> shouldRejectPairFunc)
        {
            ...
            while (true)
            {
                ...
                {
                    throw new ApplicationException("Evaluated past the end of the array.");
                }
        ...
        private int MoveBackward(int i, ref bool consistent)
        {
            ...
            var max = new Func<IEnumerable<int>, int>(enumerable => enumerable.IsEmpty() ? 0 : enumerable.Max());
            ...
            consistent = !currentDomains[h].IsEmpty();
