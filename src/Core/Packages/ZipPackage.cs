...
namespace NuGet
{
    public class ZipPackage : LocalPackage
    {
        ...
        public override void ExtractContents(IFileSystem fileSystem, string extractPath)
        {
            using (Stream stream = _streamFactory())
            {
                ...
                foreach (var part in package.GetParts()
                    .Where(p => IsPackageFile(p, package.PackageProperties.Identifier, package.PackageProperties.Version)))
                {
            ...
            else
            {
                using (Stream stream = _streamFactory())
                {
                    ...
                    fileFrameworks = from part in package.GetParts()
                                     where IsPackageFile(part, package.PackageProperties.Identifier, package.PackageProperties.Version)
        ...
        internal static bool IsPackageFile(PackagePart part, string packageId, string packageVersion)
        {
            ...
            return !ExcludePaths.Any(p => directory.StartsWith(p, StringComparison.OrdinalIgnoreCase)) &&
                   !PackageHelper.IsManifest(path, packageId, packageVersion);
        }
